...
import { GAME_CONFIG, DEFAULT_VALUES, TIME_ATTACK_CONFIG } from '../constants/gameConfig';

/**
 * Create initial game state
 */
export function createInitialGameState(seed: number = DEFAULT_VALUES.SEED): GameState {
...
export function gameReducer(state: GameState, action: GameAction): GameState {
  ...
  switch (action.type) {
    ...
    case 'MOVE_LEFT': {
      ...
      const leftPosition = { x: state.blockPosition.x - 1, y: state.blockPosition.y };
      if (isValidPosition(state.board, state.currentBlock, leftPosition) === 'valid') {
        return {
          ...state,
    ...
    case 'MOVE_RIGHT': {
      ...
      const rightPosition = { x: state.blockPosition.x + 1, y: state.blockPosition.y };
      if (isValidPosition(state.board, state.currentBlock, rightPosition) === 'valid') {
        return {
          ...state,
    ...
    case 'ROTATE_CW': {
      ...
      const cwRotation = ((state.currentBlock.rotation + 1) % 4) as 0 | 1 | 2 | 3;
      if (isValidPosition(state.board, state.currentBlock, state.blockPosition, cwRotation) === 'valid') {
        return {
          ...state,
          currentBlock: {
            ...state.currentBlock,
    ...
    case 'ROTATE_CCW': {
      ...
      const ccwRotation = ((state.currentBlock.rotation - 1 + 4) % 4) as 0 | 1 | 2 | 3;
      if (isValidPosition(state.board, state.currentBlock, state.blockPosition, ccwRotation) === 'valid') {
        return {
          ...state,
          currentBlock: {
            ...state.currentBlock,
    ...
    case 'SOFT_DROP': {
      ...
      const softDropPosition = { x: state.blockPosition.x, y: state.blockPosition.y + 1 };
      if (isValidPosition(state.board, state.currentBlock, softDropPosition) === 'valid') {
        return {
          ...state,
    ...
    case 'HARD_DROP': {
      ...
      const dropPosition = findDropPosition(state.board, state.currentBlock, state.blockPosition);
      const newState = {
        ...state,
    ...
    case 'TICK': {
      ...
      if (newState.dropTimer >= newState.dropInterval) {
        const dropPos = { x: newState.blockPosition.x, y: newState.blockPosition.y + 1 };
        
        if (isValidPosition(newState.board, newState.currentBlock, dropPos) === 'valid') {
 ...
 */
function placeCurrentBlock(state: GameState, frame: number, rng: SeededRNG): GameState {
  // Place block on board
  const newBoard = placeBlockOnBoard(state.board, state.currentBlock, state.blockPosition);
  ...
  if (rectangles.length > 0) {
    const { newBoard: clearedBoard } = clearRectanglesAndApplyGravity(newBoard, rectangles);
