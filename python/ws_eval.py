...
def compute_color_pattern_board(board, color: int) -> np.ndarray:
    ...
    Mirrors env._build_color_pattern_channel() exactly."""
    counts = np.zeros((BOARD_HEIGHT, BOARD_WIDTH), dtype=np.float32)
    for r in range(BOARD_HEIGHT - 1):
        for c in range(BOARD_WIDTH - 1):
            v = board[r][c]
            if v == color and v == board[r + 1][c] == board[r][c + 1] == board[r + 1][c + 1]:
...
def obs_to_numpy(obs_json: dict) -> dict:
    ...
    return {
        "light_board": compute_color_board(board_list, 1),
        "dark_board": compute_color_board(board_list, 2),
        "light_pattern_board": compute_color_pattern_board(board_list, 1),
        "dark_pattern_board": compute_color_pattern_board(board_list, 2),
